...
func DefaultSocketPath() (string, error) {
	...
	default:
		...
		if runtimeDir := os.Getenv("XDG_RUNTIME_DIR"); runtimeDir != "" {
			return filepath.Join(runtimeDir, "lockplus-"+uidStr+".sock"), nil
		}

		if runtime.GOOS == "darwin" {
			// macOS: use /var/run/lockplus/ — the helper runs as root (via
			...
			return "/var/run/lockplus/lockplus.sock", nil
		}
		...
		dir := filepath.Join("/tmp", "lockplus-"+uidStr)
		...
		return filepath.Join(dir, "lockplus.sock"), nil
	}
}